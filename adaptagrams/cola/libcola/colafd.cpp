...
void ConstrainedFDLayout::computeDescentVectorOnBothAxes(
    ...
    static void dumpGraphToCSV(const std::valarray<double>& X,
    ...
    static void normalizeCovarianceAspect(valarray<double>& X, valarray<double>& Y, double targetAR)
{
    ...
    if (std::abs(curAR - targetAR) < 1e-3) return;
    // Current aspect ratio
    // double minX = X.min(), maxX = X.max();
    // double minY = Y.min(), maxY = Y.max();
    // double curAR = (maxX - minX) / (maxY - minY);



    //if (std::abs(curAR - targetAR) < 0.2) return;  // already close enough
    //if (std::fabs(curAR - targetAR) / targetAR < 0.20) return;
    ...
    // double sx = (scale);
    // double sy = 1.0 / sx;
    // std::cout<<"currAR ("<<curAR<<")"<< std::endl;
    // std::cout<<"centroid ("<<meanX<<","<<meanY<<")"<< std::endl;
...
void ConstrainedFDLayout::run(const bool xAxis, const bool yAxis)
{
    ...
    // const std::string SNAPSHOT_DIR = "Output_Snapshots";
    // if (!fs::exists(SNAPSHOT_DIR)) fs::create_directory(SNAPSHOT_DIR);

    std::vector<cola::Edge> edges;  // local edge list
    edges.reserve(neighbours.size());
    for (unsigned i = 0; i < n; ++i) {
        for (unsigned j = i + 1; j < n; ++j) {
            if (neighbours[i][j] == 1)  // connected pair
    ...
    // After each layout iteration:
    // globalMinX = std::min(globalMinX, X.min());
    // globalMaxX = std::max(globalMaxX, X.max());
    // globalMinY = std::min(globalMinY, Y.min());
    // globalMaxY = std::max(globalMaxY, Y.max());
    ...
    do {
        ...
        fname << "Output_Snapshots/iter_" << std::setw(3) << std::setfill('0') << iteration++ << ".csv";
        dumpGraphToCSV(X, Y, edges, fname.str());
...
// double ConstrainedFDLayout::computeKruskalStress()  {
//     FILE_LOG(logDEBUG) << "ConstrainedFDLayout::computeKruskalStress()";
//     double numerator = 0.0;
//     double denominator = 0.0;
//
//     for (unsigned i = 0; i < n; ++i) {
//         for (unsigned j = i + 1; j < n; ++j) {
//             if (G[i][j] == 0) continue; // disconnected pair
//
//             double dx = X[i] - X[j];
//             double dy = Y[i] - Y[j];
