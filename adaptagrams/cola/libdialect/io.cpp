...
Graph_SP dialect::buildGraphFromTglf(istream &in) {
    ...
    for (auto& [id, node] : graph->getNodeLookup()) {
        double x = node->getCentre().x;
        double y = node->getCentre().y;
        node->setCentre(std::round(x), std::round(y));
    }

    for (auto& [id, edge] : graph->getEdgeLookup()) {
        auto route = edge->getRoute();
        for (auto& p : route) {
            p.x = std::round(p.x);
            p.y = std::round(p.y);
        }
