...
string Graph::writeTglf(bool useExternalIds) const {
    std::map<id_type, unsigned> id2new;
    std::ostringstream ss, edges_ss;


    unsigned next_id = 0;
    for (auto const& pair : m_nodes) {
        id_type old_id = pair.first;
        id2new[old_id] = next_id++;
    }

    //nodes
    for (auto const& pair : m_nodes) {
        id_type old_id = pair.first;
        unsigned new_id = id2new.at(old_id);
    ...
    // edges
    for (auto const& p : m_edges) {
        Edge_SP e = p.second;
        id_type sid = e->getSourceEnd()->id();
        id_type tid = e->getTargetEnd()->id();

        unsigned new_sid = id2new.at(sid);
        unsigned new_tid = id2new.at(tid);

        edges_ss << new_sid << " " << new_tid << e->writeRouteTglf() << "\n";
    }

    // Put it all together.
    std::string edges_tglf = edges_ss.str();
    std::string sepcos_tglf = m_sepMatrix.writeTglf(id2new);

    bool have_edges = !edges_tglf.empty();
    bool have_sepcos = !sepcos_tglf.empty();
...
// double Graph::computeKruskalStress() {
//
//     // Then ask it to compute the stress.
//     return m_cfdl->computeKruskalStress();
// }
