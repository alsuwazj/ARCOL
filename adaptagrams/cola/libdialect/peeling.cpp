...
//        //Z
//        std::set<Node_SP> affectedNodes;
//
//        // Identify the single parent of each leaf and increase the size of its bbox before removal
//        for (auto &leafPair : leaves) {
//            Node_SP leaf = leafPair.second;
//            Nodes neighbors = leaf->getNeighbours();
//
//            // A leaf should have exactly one neighbor (its parent)
//            COLA_ASSERT(neighbors.size() == 1);
//
//            Node_SP parent = neighbors.front(); // Get the only neighbor
//            affectedNodes.insert(parent);
//
//            // Increase padding before removing the leaf
//            // Get the current bounding box of the neighbor
//            BoundingBox bbox = parent->getBoundingBox();
//            BoundingBox childBbox = leaf->getBoundingBox();
//
//            // Incrementally increase width and height
//
//            double increaseStepX = childBbox.w() ; // Use 50% of the child's width
//            double increaseStepY = childBbox.h() ; // Use 50% of the child's height
//
////            double increaseStepX = bbox.w(); //
////            double increaseStepY = bbox.h();
//            bbox.x -= increaseStepX / 2.0;
//            bbox.X += increaseStepX / 2.0;
//            bbox.y -= increaseStepY / 2.0;
//            bbox.Y += increaseStepY / 2.0;
//
//            // Apply the new bounding box to the node
//            parent->setBoundingBox(bbox.x,bbox.X,bbox.y,bbox.Y);
//        }
